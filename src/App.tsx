...
function AppLayout() {
  ...
  const hideNav = location.pathname.startsWith("/lab/") || location.pathname.startsWith("/settings/") || location.pathname === "/upgrade";
  return (
    <AuthGate>
      <Suspense fallback={<div className="min-h-screen bg-slate-900" />}>
        <Routes>
          ...
          <Route path="/settings/privacy" element={<LegalTextViewer title="Privacy Policy" content={privacyPolicy} />} />
          <Route path="/settings/terms" element={<LegalTextViewer title="Terms of Service" content={termsOfService} />} />
          <Route path="/settings/disclaimer" element={<LegalTextViewer title="Disclaimer" content={disclaimer} />} />
...
export default function App() {
  useEffect(() => {
    if (Capacitor.isNativePlatform()) {
      Purchases.configure({ apiKey: REVENUECAT_API_KEY }).catch((err) =>
        console.error("[TechForge] RevenueCat init failed:", err)
      );
    }
  }, []);
  ...
  useEffect(() => {
    trackAppOpened();
    const listener = CapApp.addListener("pause", () => { trackSessionEnd(); });
    const handleUnload = () => trackSessionEnd();
    window.addEventListener("beforeunload", handleUnload);
    return () => { listener.then((l) => l.remove()); window.removeEventListener("beforeunload", handleUnload); };
  }, []);
