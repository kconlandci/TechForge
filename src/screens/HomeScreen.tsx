      ...
      <div className="min-h-screen bg-slate-900 p-4 pb-24">
        <div className="max-w-lg mx-auto pt-6">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h1 className="text-xl font-bold text-white">TechForge</h1>
              <p className="text-xs text-slate-400">IT Decision Simulator</p>
            </div>
          </div>
          <div className="flex items-center gap-4 mb-6 px-1">
            ...
            <div className="text-xs text-slate-500">{completedCount}/{publishedLabs.length} completed</div>
          </div>
          {activePath && (
            <section className="mb-6">
              <div className="bg-slate-800 rounded-xl p-4">
                <div className="flex items-center justify-between mb-2">
                  <h2 className="text-sm font-semibold text-white">Continue: {activePath.path.name}</h2>
                  <span className="text-[10px] text-slate-400">{activePath.completedCount}/{activePath.path.labIds.length}</span>
                </div>
                ...
                <div className="flex items-center justify-between">
                  <span className="text-xs text-slate-400">{labCatalog.find((l) => l.id === activePath.nextLabId)?.title ?? "Next lab"}</span>
                  <button onClick={() => navigate(`/lab/${activePath.nextLabId}`)}
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-sky-500 text-white text-xs font-medium min-h-[36px] active:bg-sky-600 transition-colors"
          ...
          <section className="mb-8">
            <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wide mb-3">Learning Paths</h2>
          ...
          {allFreeComplete && !isPremium && (
            <Link to="/upgrade" className="block bg-sky-500/10 border border-sky-500/30 rounded-xl p-4 mb-6 active:bg-sky-500/20 transition-colors">
              <p className="text-sm text-sky-200 text-center">Ready for more? Unlock advanced labs with TechForge Premium.</p>
            </Link>
          )}
          <section className="mb-8">
            <button onClick={() => setShowAllLabs(!showAllLabs)} className="flex items-center gap-2 w-full mb-3 min-h-[44px]">
              <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wide">All Labs</h2>
              <ChevronDown size={14} className={`text-slate-500 transition-transform ${showAllLabs ? "rotate-180" : ""}`} />
              <span className="text-[10px] text-slate-500 ml-auto">{publishedLabs.length} labs</span>
            </button>
            {showAllLabs && (
              <>
                ...
                {premiumLabs.length > 0 && (
                  <>
                    <h3 className="text-xs text-slate-500 mb-2 px-1">Premium</h3>
...
function PathCard({ path, isLabCompleted, getLabProgress, isPremium }: {
  ...
  return (
    <div className="bg-slate-800 rounded-xl overflow-hidden">
      <button onClick={() => setExpanded(!expanded)} className="w-full p-4 flex items-center gap-3 min-h-[56px] active:bg-slate-750 transition-colors text-left">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mt-0.5">
            ...
            <span className="text-[10px] text-slate-500">{completedCount}/{total}</span>
          </div>
          ...
          {path.labIds.map((labId, idx) => {
            ...
            return (
              <Link key={labId} to={isLocked ? "/upgrade" : `/lab/${labId}`}
                className={`flex items-center gap-2 py-2 px-2 rounded-lg min-h-[44px] ${isLocked ? "opacity-60" : "active:bg-slate-700"} transition-colors`}
              >
                ...
                <span className="text-xs text-white flex-1 truncate">{lab.title}</span>
                {lp?.completed && <span className="text-[10px] text-green-400">{lp.bestScore}</span>}
                {isLocked ? <Lock size={12} className="text-slate-500 shrink-0" /> : <ChevronRight size={14} className="text-slate-600 shrink-0" />}
              </Link>
            );
          })}
        </div>
...
function LabCard({ lab, locked, labProgress }: LabCardProps) {
  ...
  return (
    <Link to={locked ? "/upgrade" : `/lab/${lab.id}`} className={`block bg-slate-800 rounded-xl p-3 transition-colors ${locked ? "opacity-70 active:opacity-80" : "active:bg-slate-700"}`}>
      <div className="flex items-center justify-between">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-0.5">
            <span className="text-xs font-medium text-white truncate">{lab.title}</span>
          </div>
          <div className="flex items-center gap-2">
            <span className={`px-1.5 py-0.5 rounded text-[10px] font-medium ${tierColor}`}>{lab.tier}</span>
            <span className="text-[10px] text-slate-500">~{lab.estimatedMinutes} min</span>
        ...
        {locked ? <Lock size={14} className="text-slate-500 shrink-0" /> : <ChevronRight size={16} className="text-slate-600 shrink-0" />}
      </div>
    </Link>
  );
}
