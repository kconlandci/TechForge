      ...
      <div className="max-w-lg mx-auto pt-6">
        ...
        <div className="bg-slate-800 rounded-xl p-4 mb-4">
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-2"><Zap size={18} className="text-sky-400" /><span className="text-sm font-semibold text-white">Level {level}</span></div>
          ...
          <p className="text-[10px] text-slate-500 mt-1">{200 - (progress.xp % 200)} XP to next level</p>
        </div>
        ...
        <div className="bg-slate-800 rounded-xl p-4 mb-6">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-sm font-bold text-white">{completedCount}/{totalLabs}</span>
          </div>
          <div className="h-2 bg-slate-700 rounded-full overflow-hidden">
        ...
        {completedLabs.length > 0 && (
          <div>
            <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wide mb-3">Completed Labs</h2>
            <div className="space-y-2">
              {completedLabs.map((lp) => {
                const manifest = labCatalog.find((l) => l.id === lp.labId);
                return (
                  <div key={lp.labId} className="bg-slate-800 rounded-xl p-3 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <Cpu size={14} className="text-green-400" />
                      <span className="text-sm text-white">{manifest?.title ?? lp.labId}</span>
                    </div>
        ...
        {completedLabs.length === 0 && (
          <p className="text-sm text-slate-500 text-center py-8">Complete a lab to see your progress here.</p>
        )}
      </div>
    </div>
  );
