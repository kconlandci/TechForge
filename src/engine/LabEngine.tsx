      ...
      <div className="min-h-screen bg-slate-900 p-4">
        <div className="max-w-lg mx-auto pt-8">
          ...
          <button
            ...
            className="w-full py-3.5 rounded-xl bg-sky-500 hover:bg-sky-600 text-white font-semibold text-lg transition-colors flex items-center justify-center gap-2"
          >
    ...
    return (
      <div className="min-h-screen bg-slate-900 p-4">
        <div className="max-w-lg mx-auto pt-8">
          ...
          <div className="bg-slate-800 rounded-xl p-4 mb-6">
            <h2 className="text-sm font-semibold text-slate-400 uppercase tracking-wide mb-3">Breakdown</h2>
            <div className="space-y-2 text-sm">
              <div className="flex justify-between text-green-400"><span>Perfect answers</span><span>{perfectCount}</span></div>
              <div className="flex justify-between text-yellow-400"><span>Partial answers</span><span>{partialCount}</span></div>
          ...
          <div className="bg-slate-800 rounded-xl p-4 mb-6">
            ...
            <div className="flex flex-wrap gap-2">
              {manifest.toolRelevance.map((tool, i) => (
                <span key={i} className="px-2 py-1 rounded bg-slate-700 text-xs text-slate-300">{tool}</span>
              ))}
            </div>
          </div>
          {renderAfterResults?.()}
      ...
      <div className="sticky top-0 z-10 bg-slate-900/95 backdrop-blur border-b border-slate-800 px-4 py-3">
        <div className="max-w-lg mx-auto flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium text-white truncate max-w-[180px]">{manifest.title}</span>
          </div>
          <div className="flex items-center gap-3">
            <span className="text-xs text-slate-400">{scenarioIndex + 1}/{manifest.scenarios.length}</span>
      ...
      {phase === "feedback" && (
        <div className="sticky bottom-0 z-10 bg-slate-900/95 backdrop-blur border-t border-slate-800 px-4 py-3">
          <div className="max-w-lg mx-auto">
            <button
              onClick={advance}
              className="w-full py-3 rounded-xl bg-sky-500 hover:bg-sky-600 text-white font-semibold transition-colors flex items-center justify-center gap-2"
            >
