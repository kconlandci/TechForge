...
export default function TriageRemediateRenderer({ scenario, onSubmit, phase, feedbackResult }: RendererProps) {
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="space-y-2">
          {s.evidence.map((ev, i) => (
            <div key={i} className="bg-slate-800/50 rounded-xl p-3 border border-slate-700">
              <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">{ev.type}</span>
      ...
      <div>
        <h3 className="text-sm font-semibold text-slate-400 uppercase tracking-wide mb-2">Step 1 — Classify the Issue</h3>
            ...
            return (
              <button key={c.id} onClick={() => !isLocked && setSelectedClassification(c.id)} disabled={isLocked}
                className={`min-h-[44px] p-3 rounded-xl border-2 text-left transition-all ${
                  isSelected ? "bg-blue-500/20 border-blue-500 text-blue-200" : "bg-slate-800 border-slate-700 text-slate-300 hover:border-slate-500"
      ...
      <div>
        <h3 className="text-sm font-semibold text-slate-400 uppercase tracking-wide mb-2">Step 2 — Select Remediation</h3>
        <div className="grid gap-2">
      ...
      <div>
        <h3 className="text-sm font-semibold text-slate-400 uppercase tracking-wide mb-2">Step 3 — Justify Your Decision</h3>
        <div className="grid gap-2">
          {s.rationales.map((r) => {
