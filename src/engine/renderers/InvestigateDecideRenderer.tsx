...
export default function InvestigateDecideRenderer({ scenario, onSubmit, phase, feedbackResult }: RendererProps) {
  ...
  return (
    <div className="space-y-4">
            ...
            return (
              <div key={src.id} className="rounded-xl border-2 border-slate-700 bg-slate-800 overflow-hidden">
                <button onClick={() => toggleSource(src.id)} disabled={isLocked}
                  className={`w-full min-h-[44px] px-4 py-3 flex items-center justify-between text-left transition-all ${isLocked ? "cursor-default" : "cursor-pointer active:bg-slate-700/50"}`}
                >
                  <div className="flex items-center gap-2">
                    {isViewed ? <Eye size={14} className="text-sky-400" /> : <EyeOff size={14} className="text-slate-500" />}
                    <span className={`text-sm font-medium ${isViewed ? "text-white" : "text-slate-400"}`}>{src.label}</span>
                  </div>
                ...
                {isExpanded && (
                  <div className="px-4 pb-3 border-t border-slate-700">
                    <pre className="text-xs text-green-400 font-mono whitespace-pre-wrap leading-relaxed mt-2 bg-slate-900/50 rounded-lg p-3">{src.content}</pre>
                  </div>
                )}
              </div>
            );
      ...
      <div>
        <h3 className="text-sm font-semibold text-slate-400 uppercase tracking-wide mb-2">Choose Action</h3>
            ...
            return (
              <button key={action.id} onClick={() => !isLocked && setSelectedAction(action.id)} disabled={isLocked}
                className={`min-h-[44px] p-3 rounded-xl border-2 text-left text-sm font-medium transition-all ${
                  isSelected ? "bg-sky-500/20 border-sky-500 text-sky-200" : "bg-slate-800 border-slate-700 text-slate-300 hover:border-slate-500"
      ...
      <div>
        <h3 className="text-sm font-semibold text-slate-400 uppercase tracking-wide mb-2">Justify Your Decision</h3>
        <div className="grid gap-2">
          {s.rationales.map((r) => {
      ...
      {phase === "active" && (
        <button onClick={handleSubmit} disabled={!selectedAction || !selectedRationale}
          className="w-full py-3 rounded-xl bg-sky-500 hover:bg-sky-600 disabled:bg-slate-700 disabled:text-slate-500 text-white font-semibold transition-colors"
          ...
          <div className={`rounded-xl p-4 border-2 ${
            ...
            <div className="flex items-center gap-2 mb-2">
              {feedbackResult.type === "perfect" ? <CheckCircle size={18} className="text-green-400" /> :
               feedbackResult.type === "partial" ? <CheckCircle size={18} className="text-yellow-400" /> :
               <XCircle size={18} className="text-red-400" />}
              <span className={`text-sm font-semibold capitalize ${
                feedbackResult.type === "perfect" ? "text-green-400" : feedbackResult.type === "partial" ? "text-yellow-400" : "text-red-400"
              }`}>
