...
export default function ToggleConfigRenderer({ scenario, onSubmit, phase, feedbackResult }: RendererProps) {
  ...
  return (
    <div className="space-y-4">
        ...
        {s.items.map((item) => {
          ...
          return (
            <div key={item.id} className={`rounded-xl border-2 p-4 transition-all ${
              showResult ? isCorrect ? "border-green-500/40 bg-green-500/5" : "border-red-500/40 bg-red-500/5" : "border-slate-700 bg-slate-800"
            }`}>
              <div className="flex items-center justify-between mb-1">
                ...
                <button onClick={() => cycleState(item.id, item.states)} disabled={isLocked}
                  className={`min-w-[88px] min-h-[44px] px-4 py-2 rounded-lg text-xs font-bold uppercase transition-all ${
                    currentVal === item.states[0] ? "bg-yellow-500/20 border border-yellow-500/50 text-yellow-300" : "bg-slate-700 border border-slate-600 text-slate-300"
      ...
      {phase === "active" && (
        <button onClick={handleSubmit} className="w-full py-3 rounded-xl bg-sky-500 hover:bg-sky-600 text-white font-semibold transition-colors">
